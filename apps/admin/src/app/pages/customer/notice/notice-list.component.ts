        ...
        <table class="table table-zebra">
          <thead>
            <tr>
              <th class="w-16">번호</th>
              <th>제목</th>
              <th class="w-24 text-center">고정</th>
...
export class NoticeListComponent implements OnInit {
  ...
  togglePin(item: NoticeDTO) {
    this.noticeApi.togglePin(item.id).subscribe(updated => {
  ...
  toggleExpose(item: NoticeDTO) {
    this.noticeApi.toggleExpose(item.id).subscribe(updated => {
      this.result.update(r => r ? {
        ...r,
  ...
  remove(item: NoticeDTO) {
    ...
    this.noticeApi.remove(item.id).subscribe(() => this.load());
  }
}
